...
def current_user_from_token(token: str = Depends(oauth2_scheme),db_session: Session = Depends(get_db)):
    ...
    try:
        ...
        raise credentials_exception
    user = user_model.get_user_by_email(db_session=db_session, email=email)
    if user is None:
        raise credentials_exception
    return user
